...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($route_name === '' || $origin === '' || $destination === '' || $total_seats < 1) {
        $error = 'All fields are required and total seats must be at least 1.';
        $route = array_merge($route, compact('route_name', 'origin', 'destination', 'price', 'total_seats', 'image_url'));
    } elseif ($uploadError) {
        $error = $uploadError;
    } else {
        if ($newImageUrl) {
        ...
        $stmt = $conn->prepare('UPDATE routes SET route_name=?, origin=?, destination=?, price=?, total_seats=?, image_url=? WHERE id=?');
        $stmt->bind_param('sssdisi', $route_name, $origin, $destination, $price, $total_seats, $image_url, $id);
        $stmt->execute();
        $stmt->close();
...
<p class="form-hint">To change departure times, use "Edit" or "+ Add departure time" next to this route on the <a href="routes.php">Routes</a> page.</p>
